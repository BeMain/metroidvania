...
enum State {NORMAL, SNEAK, DUCK, STOMP, WHISTLE}
var state: State : 
	get: return _state
@onready var _state = State.NORMAL
		...
		State.NORMAL:
			if not is_on_floor():
				...
				if Input.is_action_just_pressed("whistle"):
					_state = State.WHISTLE
					body.create_pulsation(3*PI/4*body.direction)
				if Input.is_action_just_pressed("down"):
					_state = State.DUCK
